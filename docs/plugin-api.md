...
`addZarrLayer` renders a Zarr store (Zarr v2/v3 over HTTP, or — through a custom store — a kerchunk-backed cloud NetCDF or an Icechunk repository) through **GeoLibre's own** `@carbonplan/zarr-layer` instance and mirrors the result into the Layers panel. It is the Zarr counterpart of `addCogLayer`.
